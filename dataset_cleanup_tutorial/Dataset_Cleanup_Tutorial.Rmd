...
- GSE156037 -- convert to gene stable IDs to transcript stable IDs
...
```{r set directory}
knitr::opts_knit$set(root.dir = '~/bioinformatics/group-projects-invivo-fall-project/')
```
...
setwd("~/bioinformatics/group-projects-invivo-fall-project/")
...
```{r}
ID_table <- read.delim("~/bioinformatics/group-projects-invivo-fall-project/Cleaned_Datasets_transcript_stableID/celegans_cleaned_mart_ensembl.102.tsv", sep = "\t")
head(ID_table)
```
...
GSE.156037_converted  # check which columns you want to keep, i.e. only transcript stable ID and raw counts columns
...
Select only counts columns and merge samples with different numbers of genes. The IDs are already in Transcript Stable IDs
```{r GSE41205 cleanup}
# check
head(GSE.41205_N2)
head(GSE.41205_KU25)
...
Select only counts columns and merge samples with the same number of genes. The IDs are already in Transcript Stable IDs
```{r GSE63528 cleanup}
# check
head(GSE.63528_1)
...
### Conclusion
Once the cleaned datasets have been created, they can be mixed and matched and further processed for WGCNA using the pre-processing script.
