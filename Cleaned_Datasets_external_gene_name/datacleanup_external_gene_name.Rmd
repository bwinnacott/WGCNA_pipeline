...
```{r GSE134196, results='hide'}
GSE.134196 <- read.csv("Selected_GEO_datasets/GSE134196_counts.L4440.Y37A1B_5.csv", row.names = 1)
GSE.134196
```
<!-- wormbase_gene -->
<!-- 46,739 -->
...
```{r GSE156037}
GSE.156037 <- read.csv("Selected_GEO_datasets/GSE156037_raw_counts.csv", row.names = 1)
GSE.156037
```
<!-- wormbase_gene -->
<!-- 46,093 -->
...
GSE.134196_IDs <- getBM(attributes=c('wormbase_gene', 'wormbase_cds'), 
                filters='wormbase_gene',
                values=rownames(GSE.134196),
                mart=celegans_mart)
GSE.134196_IDs
length(unique(GSE.134196_IDs$external_gene_name))

### initial number of genes 46,739
# external_gene_name      46,577    aap-1
# wormbase_cds            55,330    Y110A7A.10
# ensembl_transcript_id   61,081    Y110A7A.10.1
# wikigene_name           46,571    aap-1
# 

length(unique(GSE.134196_IDs$wormbase_gene))
# 46571
# maybe do wormbase cds then cleanup ending a,b,c,d, etc.?
```

```{r WB to EGN GSE.134196}
# check whether IDs in rownames or in a column
GSE.134196
...
# check whether IDs in rownames or a column and check initial gene count
invivo
...
GSE.134196 <- rownames_to_column(GSE.134196, var = "wormbase_gene")
GSE.156037 <- rownames_to_column(GSE.156037, var = "wormbase_gene")
...
```{r column cleanup}
# GSE46257
# select only the counts columns, pulled IDs out from rownames
GSE.46257 <- GSE.46257 %>%
  select(ends_with("counts")) %>% 
