...
require("knitr")
opts_knit$set(root.dir = "/Users/brendanwinnacott/bioinformatics/Bi610_lab/group-projects-invivo-fall-project/WGCNA")
fig.dim <- 6
...
datExpr <- read.csv("raw_wormspellGEO_transcript_stable_ID.csv",row.names = gene_ids)
datExpr$X <- NULL
# "head" the file to preview it
head(datExpr)
```

Normalize the raw gene counts using edgeR's 'calcNormFactors' function.
...
datTraits <- read.csv("design_matrix.csv",row.names = 1)
...
Based on our sample dataset, a soft threshold power value of 8 is appropriate for network analysis. **Note:** 
